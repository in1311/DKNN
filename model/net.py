...
class DKNN(nn.Module):
    ...
    def kriging_decoder(self, z_know, var_know, var_unknow, trend_unknow, trend_know):
        
        ##### kriging decoder and interpolation  #####
        device = str(var_know.device) 
        k = trend_know.shape[-1]
        batch_size = var_unknow.shape[0]
        ...
        sys_mat_know[0:self.known_num, 0:self.known_num] = var_know
        ...
        sys_mat_unknow[0:batch_size, 0:self.known_num] = var_unknow
    ...
    def forward(self, input_coods, input_features, input_pe, know_coods, know_features):
        ...
        ##### SSAN #####
        var_unknow = self.Ssan(vec_unknow, vec_know, input_pe, self.pe_know)  # variance matrix of unknown points
        var_know = self.Ssan(vec_know, vec_know, self.pe_know, self.pe_know)  # variance matrix of known points
        # Zero the diagonal of the matrix var_know
        var_know = var_know - torch.diag_embed(torch.diag(var_know))
        
        ##### PGRN #####
        known_z = torch.mean(self.Pgrn(vec_know, self.pe_know),-1)  # PGRN output of known points
            
        ##### Meta-PN #####
        trend_know = self.MetaPN(know_coods, self.pe_know)  # trend vector of known points
        trend_unknow = self.MetaPN(input_coods, input_pe)  # trend vector of unknown points
        
        ##### Kriging Decoder #####
        output, out_trend = self.kriging_decoder(known_z, var_know, var_unknow, trend_unknow, trend_know)
