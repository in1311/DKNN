...
import math
from model.utils import pe_dis_cal


class DKNN(nn.Module):
    def __init__(self, d_input, d_model, d_q, d_k, d_v, known_num, d_trend, top_k, pe_weight):
        ...
        self.Ssan = SSAN(d_model, d_q, d_k, d_v, top_k, pe_weight)
        self.MetaPN = MetaPN(d_pe=self.d_pe, d_model=d_model, d_trend=d_trend)
        self.pe_know = None
        self.pe_val = None
    
    def kriging_decoder(self, z_know, var_know, var_pre, pe_pre_trend, pe_know_trend):
        ##### kriging decoder and interpolation  #####
        device = str(var_know.device) 
        k = pe_know_trend.shape[-1]
        batch_size = var_pre.shape[0]
        sys_mat_know = torch.zeros(self.known_num + k, self.known_num + k).to(device)
        sys_mat_pre = torch.zeros(batch_size, self.known_num + k).to(device)

        sys_mat_know[0:self.known_num, 0:self.known_num] = var_know
        sys_mat_know[0:self.known_num, self.known_num:self.known_num + k] = pe_know_trend
        sys_mat_know[self.known_num:self.known_num + k, 0:self.known_num] = pe_know_trend.T

        sys_mat_pre[0:batch_size, 0:self.known_num] = var_pre
        sys_mat_pre[0:batch_size, self.known_num:self.known_num + k] = pe_pre_trend
...
class SSAN(nn.Module):
    ...
    def __init__(self, d_model, q, k, v, top_k, pe_weight):
